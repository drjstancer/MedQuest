...
export default function ExplorationRuntimePage({
  params,
}: {
  params: Promise<{
    slug: string;
...
const [selectedReflection, setSelectedReflection] =
  ...
  const explorationClues = useMemo(() => {
  return clues.filter(
    (clue) => clue.explorationId === exploration?.id
  );
}, [exploration]);

  const career = careers.find(
    (item) => item.id === exploration?.featuredCareerId
  );

  const reflection = reflectionPrompts.find(
    (item) => item.explorationId === exploration?.id
  );
  const guessOptions = careers
...
saveJourneyEntry({
  ...
  const activeClue = explorationClues[currentClueIndex];
      ...
      Incorrect guess. Time penalty applied.
        ...
        {revealMode && (
          <div className="rounded-3xl border border-cyan-400/30 bg-cyan-400/[0.05] p-10">
            ...
            {exploration.recommendedNext?.length > 0 && (
  <div className="mt-12">
    ...
    <div className="grid gap-4 md:grid-cols-2">
      {exploration.recommendedNext.map((slug) => {
        const nextExploration =
          explorations.find(
            (item) => item.slug === slug
          );

        if (!nextExploration) return null;

        return (
          <Link
            key={slug}
            href={`/explorations/${slug}`}
            className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 hover:border-cyan-400/40 transition-all"
          >
            ...
            <p className="text-slate-400 text-sm">
              {nextExploration.cinematicIntro}
            </p>
          </Link>
        );
      })}
